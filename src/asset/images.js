import busan from './bank/busan.png';
import city from './bank/city.png';
import cu from './bank/cu.png';
import dgb from './bank/dgb.png';
import gwangju from './bank/gwangju.png';
import hana from './bank/hana.png';
import ibk from './bank/ibk.png';
import im from './bank/im.png';
import k from './bank/k.png';
import kakao from './bank/kakao.png';
import kb from './bank/kb.png';
import kdb from './bank/kdb.png';
import mg from './bank/mg.png';
import nh from './bank/nh.png';
import post from './bank/post.png';
import sc from './bank/sc.png';
import sh from './bank/sh.png';
import shinhan from './bank/shinhan.png';
import toss from './bank/toss.png';
import woori from './bank/woori.png';

import canada from './flag/canada.png';
import china from './flag/china.png';
import eu from './flag/eu.png';
import hongkong from './flag/hongkong.png';
import indonesia from './flag/indonesia.png';
import japan from './flag/japan.png';
import korea from './flag/korea.png';
import malaysia from './flag/malaysia.png';
import russia from './flag/russia.png';
import thailand from './flag/thailand.png';
import uk from './flag/uk.png';
import usa from './flag/usa.png';
import vietnam from './flag/vietnam.png';

import check from './icon/check.png';
import dutchpay from './icon/dutchpay.svg';
import exchange from './icon/exchange.svg';
import exchangeActive from './icon/exchangeActive.svg';
import main from './icon/main.svg';
import mainActive from './icon/mainActive.svg';
import map from './icon/map.svg';
import mapActive from './icon/mapActive.svg';
import recommendation from './icon/recommendation.svg';
import remit from './icon/remit.svg';
import remitActive from './icon/remitActive.svg';
import groupRemit from './icon/groupRemit.svg';
import tel from './icon/tel.png';
import autoRemit from './icon/autoRemit.svg';
import recruit from './icon/recruit.svg';
import condition from './icon/condition.svg';

import circleLogo from './logo/circleLogo.png';
import comingLogo from './logo/comingLogo.png';
import loading from './logo/loading.gif';
import logo from './logo/logo.png';
import nbgLogo from './logo/nbgLogo.svg';
import splash from './logo/splash.gif';

import onboarding1 from './onboarding/onboarding1.png';
import onboarding2 from './onboarding/onboarding2.png';
import onboarding3 from './onboarding/onboarding3.png';

export const Logos = {
  circleLogo,
  comingLogo,
  loading,
  logo,
  nbgLogo,
  splash,
};

export const Icons = {
  check,
  dutchpay,
  exchange,
  exchangeActive,
  main,
  mainActive,
  map,
  mapActive,
  recommendation,
  remit,
  remitActive,
  groupRemit,
  tel,
  autoRemit,
  condition,
  recruit,
};

export const Flags = {
  canada,
  china,
  eu,
  hongkong,
  indonesia,
  japan,
  korea,
  malaysia,
  russia,
  thailand,
  uk,
  usa,
  vietnam,
};

export const Banks = {
  busan,
  city,
  cu,
  dgb,
  gwangju,
  hana,
  ibk,
  im,
  k,
  kakao,
  kb,
  kdb,
  mg,
  nh,
  post,
  sc,
  sh,
  shinhan,
  toss,
  woori,
};

export const Onboardings = {
  onboarding1,
  onboarding2,
  onboarding3,
};
