...
defineEmits(['select', 'reshuffle', 'delete']);
</script>

<template>
  <div
    class="grid grid-cols-3 grid-rows-4 gap-2 w-full h-full justify-items-center"
  >
    <button
      v-for="n in numbers.slice(0, 9)"
      :key="n"
      @click="$emit('select', n)"
